...
- 🔀 [Console codepage cp1252 crash `pio upload` giữa chừng (2026-08-03)](esptool-utf8-console-crash-2026-08-03.md) — esptool in progress bar Unicode, Windows console `cp1252` không encode được → thread đọc output của PlatformIO chết → pipe nghẽn → tiến trình treo → bị timeout kill giữa lúc ghi flash. Fix: `PYTHONIOENCODING=utf-8`/`PYTHONUTF8=1` trước MỌI lệnh `pio` upload/monitor (đã vào skill `pio-dev`, không cần nhắc lại).
