...
- `checkDistance()` — máy trạng thái debounce:
  ...
  - **Hysteresis (2026-08-10)** — ngưỡng vào và ngưỡng ra khác nhau, mốc so sánh là `publishedState` (cue đang thực sự phát ra), **không** phải `lastState`:
    - đang MISSING → FULL: cần **tất cả** sensor enabled + online đều trong range (`outOfRange == 0`)
    - đang FULL → MISSING: cần `outOfRange >= effectiveThreshold` (`missingThreshold` kẹp theo `requiredCount`)
    - ở giữa: **giữ nguyên** trạng thái đang có — vùng đệm này là mục đích chính, mở cue thì đòi đủ hết nhưng khi show đang chạy thì 1 sensor lệch tạm không cắt ngang cue
    - `missingThreshold = 1` cho ra đúng hành vi cũ (không có vùng đệm)
    - Lấy mốc `lastState` là **sai**: nó còn dao động trong lúc chờ `confirmTime`, ngưỡng sẽ tự nhảy qua lại giữa "vào" và "ra" ngay trong một lần chuyển, kết quả phụ thuộc nhịp loop.
    - `requiredCount > 0` giữ ở cả 2 nhánh → mất hết sensor (offline sạch / bỏ tick hết) thì về MISSING, không bao giờ tự nhận FULL khi đang mù hoàn toàn.
    - Sensor OFFLINE bị loại khỏi `requiredCount` nên **tiêu chuẩn vào FULL tự hạ theo** khi hỏng 1 sensor (chỉ cần các con còn sống đều trong range) — có chủ đích, để show vẫn chạy được khi hỏng phần cứng.
